...
inline fun <reified O> get(endpoint: String, token: String? = null, noinline onComplete: Response<O>.() -> Unit) = Request(endpoint, RequestType.GET, token, null, typeInfo<ApiType>(), typeInfo<O>(), onComplete)
...
inline fun <reified O> delete(endpoint: String, token: String, noinline onComplete: Response<O>.() -> Unit) = Request(endpoint, RequestType.DELETE, token, null, typeInfo<ApiType>(), typeInfo<O>(), onComplete)
